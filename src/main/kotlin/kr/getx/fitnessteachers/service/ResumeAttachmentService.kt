...
class ResumeAttachmentService(
    ...
    fun syncResumeAttachment(
            resumeAttachmentDto: List<ResumeAttachmentDto>
    ): List<ResumeAttachment> {
        val existingResumeAttachments =
                resumeAttachmentRepository.findAll().associateBy { it.resumeAttachmentId }
        ...
        resumeAttachmentDto.forEach { dto ->
            if (dto.resumeAttachmentId == 0) { // id 값이 0일때 추가
                ...
                // 기존 데이터 업데이트
                val resumeAttachment =
                        existingResumeAttachments[dto.resumeAttachmentId]?.apply {
                            attachmentUrl = dto.attachmentUrl
                        }
    ...
    fun deleteResumeAttachment(resumeAttachmentId: List<Int>) {
        val resumeAttachments =
                resumeAttachmentRepository.findAllByResumeAttachmentIdInAndIsDeletedFalse(
                        resumeAttachmentId
                )
        if (resumeAttachments.isEmpty()) {
            throw IllegalArgumentException("해당 이력첨부파일이 존재하지 않습니다.")
        }
